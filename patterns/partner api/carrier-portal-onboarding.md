...
sequenceDiagram
    ...
    Integrator->>ShipStation: POST /v1/tokens/ephemeral<br/>On-Behalf-Of: accountId
    ShipStation-->>Shipper: Provide ephemeral token link
...
- The ephemeral token is generated by ShipStation and provided directly to the shipper for dashboard access. The token is single-use and short-lived.
...
- This integration flow supports enabling ShipStation API carriers and a shippers own carrier account (BYOA)
